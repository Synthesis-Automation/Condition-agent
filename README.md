...
Deterministic chemistry tools (FastAPI + RDKit-friendly) to support condition recommendation. Focuses on Ullmann C鈥揘 first, with simple hooks for more families.
...
- `POST /api/v1/featurize/molecular` (alias: `/api/v1/featurize/ullmann`) 鈥?Substrate features for Ullmann C鈥揘.
...
- `chemtools/` 鈥?Deterministic tool implementations:
  ...
  - `featurizers/molecular.py` (alias kept: `featurizers/ullmann.py`) 鈥?Ullmann C鈥揘 featurizer
