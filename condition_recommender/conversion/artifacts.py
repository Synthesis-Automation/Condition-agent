...
def _build_shared_core_companion(
    index_path: Path,
    *,
    ...
    """Use one projection builder for direct and combined libraries."""
    from ..generic_indexing import load_generic_index
    from ..shared_core_index import SharedCoreBuildCancelled, build_shared_core_index

    try:
        return build_shared_core_index(
            load_generic_index(index_path),
            index_path.with_suffix(".shared_core.sqlite"),
            workers=workers,
            cancel_check=cancel_check,
            progress_callback=progress_callback,
        )
    except SharedCoreBuildCancelled as exc:
...
def combine_saved_recommendation_batches(
    ...
    *,
    ...
    notify("shared_core_started", "Building direct and product-side core projections.")
    shared_core_report = _build_shared_core_companion(
        index_path,
        cancel_check=cancel_check,
        progress_callback=lambda count: notify(
            "shared_core_rows", f"Projected {count} observations.", count
        ),
    )
    ...
    report: Dict[str, Any] = {
        ...
        **combined,
        ...
        "artifacts": {
            ...
            "shared_core_index": _artifact_entry(
                index_path.with_suffix(".shared_core.sqlite"), destination
            ),
        },
        "shared_core_report": shared_core_report,
        "catalogs": catalogs,
        "warnings": [],
    }
...
def build_recommendation_artifacts(
    ...
    *,
    ...
    experimental_shared_core: bool | None = None,
    use_rxnmapper: bool = False,
    conversion_mode: str = "full",
    checkpoint_interval: int = 1,
    progress_callback: Optional[
    ...
    Fast-index builds include the default shared-core projections. The historical
    ``experimental_shared_core`` option remains an explicit baseline-build switch;
    ``None`` follows ``build_fast_index`` for canonical-only conversion workflows.
    """
    if experimental_shared_core is None:
        experimental_shared_core = build_fast_index
    if experimental_shared_core and not build_fast_index:
        raise ValueError("Shared-core artifacts require build_fast_index=True")
    ...
    if experimental_shared_core:
        ...
        shared_core_report = _build_shared_core_companion(
            ...
            cancel_check=cancel_check,
            progress_callback=lambda count: notify(
                "shared_core_rows", f"Projected {count} observations.", row_count=count
            ),
        )
    ...
    if shared_core_report is not None:
        artifacts["shared_core_index"] = _artifact_entry(
            destination / "generic_index.shared_core.sqlite", destination,
        )
    ...
    if shared_core_report is not None:
        report["settings"]["experimental_shared_core"] = True
        report["shared_core_report"] = shared_core_report
    report["report_path"] = str(report_path.resolve())
    _atomic_json(report_path, report)
    notify(
        "completed",
