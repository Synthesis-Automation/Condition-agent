...
def _strategy_summary_table(report: Mapping[str, Any]) -> str:
    ...
    return (
        ...
        '<table><thead><tr><th>Relationship</th><th>Patents</th><th>Occurrences</th>'
...
def _review_controls() -> str:
    return (
        ...
        '<label>Suggested relationship<select class="review-relationship">'
        '<option value="unreviewed">Unreviewed</option>'
        '<option value="handle_progression">Handle progression</option>'
        '<option value="same_site_coupled">Same-site coupled</option>'
...
def _strategy_card(item: Mapping[str, Any], index: int) -> str:
    ...
    return (
        ...
        f'<span class="relationship">{html.escape(relationship.replace("_", " "))}</span>'
...
def render_coupled_route_strategy_html(
    ...
    *,
    ...
    relationships = sorted(
        {str(item.get("relationship_class") or "") for item in sample}
    )
    ...
    storage_key = "coupled-route-strategy-review:" + str(
        report.get("source_sha256") or "unknown"
    )
    return (
        ...
        '<p>Strict pairs share a lineage-traced active atom across both physical reactions. Review pairs are local or ambiguous. Rejected controls edit independent sites. Decide whether each pair is useful as one planning strategy.</p>'
        '<div class="summary">'
        ...
        '<label>Relationship<select id="relationship-filter"><option value="all">All</option>'
