...
An optional external atom mapper can be enabled beside this authority ladder,
not inside it. Its output is a proposed correspondence with explicit
provider/model provenance and confidence. The system first proves that the
mapper preserved every input structure, then extracts typed edits and compares
them with exact reconstruction or internal correspondence hypotheses. A
generated map is never relabelled as a supplied map, and mapper confidence
alone does not make a dataset row verified.

If the external edit profile agrees with exactly one retained internal
hypothesis, that consensus can supply a review-qualified query signature. If
there is no internal hypothesis, a structurally valid mapper-only signature can
also be retained for review. Disagreement or multiple matches preserve the
original ambiguity. Converted mapper-derived rows are not normal precedents;
query-time mapper signatures may retrieve only independently verified indexed
records and the recommendation remains explicitly expert-review required.
...
Here, “valid mapping” means validated mapping supplied with the source
reaction. A mapping generated by RXNMapper or another external provider remains
provider-scoped evidence until it agrees with an independent structural source
