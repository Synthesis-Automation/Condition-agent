...
Filters:
  ...
  - quality_score >= 0.7
  - count >= 10
  - Pick best template per (cluster_id, primary_source_family) — highest quality_score
...
EXTRACTED_FILE = ROOT / "results" / "extracted_templates.json"
HTE_FILE = ROOT / "chemtools" / "taxonomy" / "data" / "hte_templates.json"
RETRON_FILE = ROOT / "chemtools" / "taxonomy" / "data" / "retron_patterns.json"
...
def _smarts_has_heteroatom(smarts: str) -> bool:
    """Check if SMARTS reactant side contains heteroatom references (N, O, S, F, Cl, Br, I, P, B, etc.).
    ...
    # Match uppercase letters that are NOT C or H (and not inside descriptors like D2, +0)
