...
def _partial_cn_label(analysis: Any) -> Tuple[str | None, str]:
    ...
    labels = sorted({
        ...
        if candidate.grammar_id == "sp2_c_n_substitution" and candidate.reaction_label
    })
    if len(labels) == 1:
...
def _admission(analysis: Any, yield_pct: float | None, conditions_complete: bool) -> Tuple[AdmissionTier, Tuple[str, ...]]:
    ...
    if analysis.family_environment is None or analysis.family_environment.family_id != "c_n_coupling":
        reasons.append("missing_c_n_family_environment")
    if analysis.product_connection is None or analysis.product_connection.connection_type != "C_N":
...
def convert_row(row: Dict[str, Any], source_row_number: int) -> RecommendationRecord:
    ...
    label, label_status = _partial_cn_label(analysis)
    return RecommendationRecord(
        reaction_id=str(row.get("reaction_id") or f"row-{source_row_number}"),
        source_row_number=source_row_number,
        reaction_smiles=str(row.get("reaction_smiles") or ""),
        admission_tier=tier, admission_reasons=reasons,
        evidence_quality=analysis.evidence_quality, named_family=analysis.named_family,
        reaction_label=label, reaction_label_status=label_status,
        yield_pct=yield_pct, temperature_c=optional_float(row.get("temperature_c")),
        time_h=optional_float(row.get("time_h")), conditions=conditions,
...
def flatten_record(record: RecommendationRecord) -> Dict[str, Any]:
    ...
    return {
        "schema_version": record.schema_version, "reaction_id": record.reaction_id,
        "source_row_number": record.source_row_number, "admission_tier": record.admission_tier.value,
        "admission_reasons": joined(record.admission_reasons), "reaction_smiles": record.reaction_smiles,
        "evidence_quality": record.evidence_quality, "reaction_label": record.reaction_label or "",
        "reaction_label_status": record.reaction_label_status,
        "yield_pct": record.yield_pct if record.yield_pct is not None else "",
        "condition_recipe_id": record.conditions.recipe_id, "conditions_complete": record.conditions.complete,
        "catalyst_cas": joined(record.conditions.catalyst_cas), "reagent_cas": joined(record.conditions.reagent_cas),
...
def convert_file(input_path: str | Path, output_dir: str | Path) -> Dict[str, Any]:
    input_path, output_dir = Path(input_path), Path(output_dir); output_dir.mkdir(parents=True, exist_ok=True)
    ...
    for tier, values in by_tier.items(): _write(output_dir / f"{tier.value}.csv", values)
    report = {
        "schema_version": "1.0", "input_file": str(input_path), "total_rows": len(records),
