...
test('larger libraries report their actual size and retrieve cyanation precedents', async ({ page }) => {
  ...
  expect(caps.recommendation_engine).toBe('shared_reaction_core.v2')
  ...
  await page.screenshot({ path: '../../results/shared_core_default/workbench_full.png', fullPage: true })
  ...
  await page.screenshot({ path: '../../results/shared_core_default/workbench_reduction.png', fullPage: true })
  await library.selectOption('compact')
  await expect(library.locator('option:checked')).toHaveText(/Compact · .* records/)
  await page.getByLabel('Reaction SMILES', { exact: true }).fill('Brc1c2c(cccc2)[nH]n1.N#C>>N#CC1=NNC2=CC=CC=C12')
  const compactPromise = page.waitForResponse(response => response.url().endsWith('/recommendations'), { timeout: 90_000 })
  await page.getByRole('button', { name: 'Recommend conditions', exact: true }).click()
  const compactResponse = await compactPromise
  expect(compactResponse.ok()).toBeTruthy()
  const compact = (await compactResponse.json()).data
  expect(compact.recommendations.length).toBeGreaterThan(0)
  expect(compact.recommendations.every((item: { match_namespace: string }) => item.match_namespace === 'shared_reaction_core.v2')).toBe(true)
