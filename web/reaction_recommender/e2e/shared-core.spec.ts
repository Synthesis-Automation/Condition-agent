...
test('workbench shows graph levels and product-side evidence without changing the query', async ({ page }) => {
  ...
  expect(result.recommendations[0].match_namespace).toBe('shared_reaction_core.v2')
  ...
  await page.screenshot({ path: '../../results/shared_core_v2/workbench_development.png', fullPage: true })
