...
- tanimoto(a, b) -> float
- drfp_tanimoto(rxn1: str, rxn2: str, n_bits: int = 4096, radius: int = 3) -> float
"""
...
_DRFP_ENCODER_INSTANCE = None


def _import_drfp():
    """Get or create the singleton DRFP encoder instance."""
    global _DRFP_ENCODER_INSTANCE
    if _DRFP_ENCODER_INSTANCE is not None:
        return _DRFP_ENCODER_INSTANCE
    
    try:
        from drfp import DrfpEncoder  # type: ignore
        _DRFP_ENCODER_INSTANCE = DrfpEncoder
        return DrfpEncoder
    except Exception:
        return None
...
# Suppress RDKit warnings globally (must be done before any RDKit operations)
try:
...
def drfp_tanimoto(rxn1: str, rxn2: str, n_bits: int = 4096, radius: int = 3) -> float:
    ...
    Example:
        >>> similarity = drfp_tanimoto('Br.N>>C', 'Cl.N>>C')
