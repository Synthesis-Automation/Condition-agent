...
def test_diversity_interleaves_groups_only_within_structural_band() -> None:
    first = _candidate(
        "a1", 0.90, operator="A", site="site-a", synthon="syn-a"
    )
    same_group = _candidate(
        "a2", 0.89, operator="A", site="site-a", synthon="syn-a"
    )
    other_group = _candidate(
        "b", 0.87, operator="B", site="site-b", synthon="syn-b"
    )
    weak = _candidate(
        "c", 0.70, operator="C", site="site-c", synthon="syn-c"
    )

    ranked = rank_operator_site_diverse(
        (same_group, weak, other_group, first)
    )
    reversed_input = rank_operator_site_diverse(
        (first, other_group, weak, same_group)
    )
