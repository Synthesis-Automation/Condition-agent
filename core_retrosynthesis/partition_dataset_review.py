...
PARTITION_DATASET_REVIEW_ALGORITHM_VERSION = "observed_route_k_way_review.v1"
...
def _case_panel(case: PartitionDatasetReviewCase) -> str:
    ...
    return (
        ...
        f"<h2>Observed route · maximum k={case.maximum_k}</h2>"
...
def render_partition_dataset_review_html(review: PartitionDatasetReview) -> str:
    ...
    coverage = "".join(
        f"<div><strong>{count:,}</strong><span>routes with max k={maximum_k}</span></div>"
...
</style></head><body><header><p>K-WAY SYNTHETIC PARTITION · OBSERVED ROUTE DATASET</p><h1>{review.sample_size}-route graphical fragmentation review</h1><p>This is an enriched functionality sample from observed routes with maximum k≥{review.minimum_selected_k}; it is not an unbiased estimate of corpus accuracy. Module colors describe target-atom ownership, while separate structures show the corresponding latent precursor states.</p><div class="metrics"><div><strong>{review.source_route_count:,}</strong><span>source routes</span></div><div><strong>{review.eligible_route_count}</strong><span>eligible max-k routes</span></div><div><strong>{review.sample_size}</strong><span>selected cases</span></div>{coverage}</div></header><main><div class="toolbar"><label>Projection status<select id="resolved"><option value="all">All</option><option value="true">Fully projected</option><option value="false">Partial projection</option></select></label><label>Maximum k<select id="max-k"><option value="all">All</option>{k_options}</select></label><button id="export">Export review JSON</button><span id="visible"></span></div><div class="warning">{_warnings(review.warnings)}</div>{''.join(_case_panel(case) for case in review.cases)}</main><script type="application/json" id="partition-dataset-review-data">{payload}</script><script>
