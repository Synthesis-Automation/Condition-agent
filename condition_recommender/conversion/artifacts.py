...
def build_recommendation_artifacts(
    ...
    *,
    ...
    if experimental_shared_core:
        ...
        notify("shared_core_started", "Building experimental direct and product-side core projections.")
        try:
            shared_core_report = build_shared_core_index(
                load_generic_index(index_path), destination / "generic_index.shared_core.sqlite",
                cancel_check=cancel_check,
                progress_callback=lambda count: notify("shared_core_rows", f"Projected {count} observations.", row_count=count),
            )
