        ...
        <div className="analysis-control-layout">
          ...
          <div className="analysis-options">
            <div className={`option-grid ${mode === 'features' || mode === 'coupled_strategy' ? 'feature-options' : ''}`}>
              ...
              {mode === 'recommendation' ? (
                <>
                  <label><span>Search scope</span><select value={searchScope} onChange={(event) => setSearchScope(event.target.value as typeof searchScope)}><option value="same_handle">Same reactive handle</option><option value="automatic">Automatic broadening</option><option value="broad">Broader analogues</option></select><small>Automatic broadening searches for the requested number of recipes. Fewer may be available within the search limits.</small></label>
                  <label className="wide-option"><span>Prioritize</span><div className="joined-control"><select value={profileId} onChange={(event) => { setProfileId(event.target.value); setCustomWeights(null) }}>{profiles.filter((profile) => ['default', 'closest_chemistry', 'evidence'].includes(profile.profile_id)).map((profile) => <option key={profile.profile_id} value={profile.profile_id}>{profile.label}</option>)}</select><button type="button" className="button quiet" onClick={() => setRankingOpen(true)} disabled={!selectedProfile}>Advanced weights{customWeights ? ' (custom)' : ''}</button></div><small>Ranks recipes within each match level; scores are not success probabilities.</small></label>
                </>
              ) : mode === 'weak_label' ? (
            ...
            {mode !== 'weak_label' && <details className="advanced-options"><summary>Advanced options</summary><div>
              {mode === 'recommendation' && <label><span>Independent evidence target</span><input type="number" min="1" max="100" placeholder="Default: 2" value={minimumPoolSize ?? ''} onChange={(event) => setMinimumPoolSize(event.target.value ? Math.min(100, Math.max(1, Number(event.target.value))) : null)} /><small>Repeated records from one reference count together.</small></label>}
