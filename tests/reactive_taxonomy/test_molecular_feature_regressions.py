...
from reactive_taxonomy.descriptors import reactivity_profile_tokens, render_reactivity_profile
...
def _fingerprint(smiles: str) -> Counter:
    analysis = analyze_molecule(smiles)
    environments = {env.hypothesis_id: env.reactivity_profile for env in analysis.reactive_site_environments}
    return Counter(
        (site.canonical_signature, reactivity_profile_tokens(environments[site.hypothesis_id]),
...
@pytest.mark.parametrize("smiles", [
    "CC=CCO", "CC#CCO", "CC=C(C)O", "COc1ccc(Br)cc1", "Clc1cccc[n+]1C",
])
...
@pytest.mark.parametrize("smiles,allylic,propargylic", [
    ("BrCC=O", False, False), ("BrCC#N", False, False),
    ("BrCC=C", True, False), ("BrCC#C", False, True),
    ("BrCOC=C", False, False),
])
def test_alkyl_activation_requires_a_carbon_carbon_pi_system(
    smiles: str, allylic: bool, propargylic: bool,
) -> None:
...
@pytest.mark.parametrize("smiles,signature", [
    ("CC#N", "PI|Nitrile"), ("CC=NC", "PI|PolarizedC=N"),
])
def test_unsupported_profiles_do_not_claim_neutral_or_open(
    smiles: str, signature: str,
) -> None:
...
def test_rdkit_helper_keeps_original_indices_including_disconnected_components() -> None:
    ...
    for site in sites:
        ...
        if site.site_type == "leaving_group":
            ...
            assert molecule.GetBondBetweenAtoms(roles["anchor"][0], roles["handle"][0]) is not None
...
def test_context_opt_out_skips_descriptor_construction(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_aromatic_resonance_requires_direct_attachment_and_correct_position() -> None:
    ...
    assert any(c.pathway == "resonance" and c.positional_relation == "para" for c in para.contributions)
    assert not any(c.pathway == "resonance" for c in meta.contributions)
    assert not any(c.source_id == "aromatic_substituent:alkoxy" for c in remote.contributions)
...
def test_aromatic_rule_validation_rejects_missing_roles_and_invalid_weights() -> None:
    ...
    from reactive_taxonomy.descriptors.registry import descriptor_rules, validate_aromatic_substituent_rules
