...
def get_available_family_npz_files(data_dir: str = "data/reaction_dataset") -> List[str]:
    ...
    Automatically discovers all reaction type NPZ files generated by the batch processor.
    """
    data_path = Path(data_dir)
    if not data_path.exists():
        return []
    
    # Find all *_drfp.npz files except unified/combined files
    npz_files = []
    exclude_patterns = ["unified", "all_families", "combined", "index"]
    
    for npz_file in data_path.glob("*_drfp.npz"):
        # Skip unified/combined index files
        if any(pattern in npz_file.name.lower() for pattern in exclude_patterns):
            continue
...
def load_family_npz(npz_path: str) -> Dict[str, np.ndarray]:
    ...
    fingerprints = data.get('fps', data.get('fingerprints', []))
    ...
    for rid, fp in zip(reaction_ids, fingerprints):
        # Ensure consistent uint8 format (handles both matrix and object arrays)
