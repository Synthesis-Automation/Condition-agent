...
def test_reaction_catalog_expands_motif_sets() -> None:
    ...
    assert "Ar-OTf" in suzuki.reactants["electrophiles"]
    assert "Ar-B(OH)2" in suzuki.reactants["nucleophiles"]

    sonogashira = definitions["sonogashira"]
    assert "Ar-OMs" in sonogashira.reactants["electrophiles"]
    assert "Ar-F" not in sonogashira.reactants["electrophiles"]

    snar = definitions["snar_cn"]
    assert "Ar-NO2" in snar.reactants["activators"]
    assert "Pyridine" in snar.reactants["activators"]
    ...
    assert "R-OTs" in sn2.reactants["electrophiles"]

    reductive = definitions["reductive_amination"]
    assert "Any-CHO" in reductive.reactants["electrophiles"]

    rcm = definitions["ring_closing_metathesis"]
    assert "Any-CHCH2" in rcm.reactants["nucleophiles"]
