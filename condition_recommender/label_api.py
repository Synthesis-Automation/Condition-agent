    ...
    for role, site in selected.role_assignments.items():
        ...
        participants.append(
            QueryParticipant(
                ...
                attachment_classes=tuple(sorted({
                    str(item["attachment_carbon_class"])
                    for item in attached
                    if item.get("attachment_carbon_class")
                })),
                alpha_branched=any(bool(value) for value in alpha_values) if alpha_values else None,
            )
        )
...
def _qualifier_similarity(query: QueryParticipant, precedent: LabelParticipant) -> float:
...
def _pair_similarity(
    ...
    for order in ((0, 1), (1, 0)):
        signatures = [
            _signature_similarity(query[index].signature, precedent[order[index]].signature)
...
def _rank_recipes(
    ...
    *,
    ...
    scored = []
    for row in rows:
        label_score, signature_score, qualifier_score = _pair_similarity(query, row.participants)
        if signature_score <= 0.0:
            continue
        scored.append((label_score, signature_score, qualifier_score, row))

    groups: Dict[str, List[Tuple[float, float, float, LabelIndexedReaction]]] = defaultdict(list)
    for item in scored:
    ...
    for recipe_id, members in groups.items():
        members.sort(key=lambda item: (-item[0], -item[3].yield_pct, item[3].source_row_number))
        ...
        expected_yield = sum(
            weight * item[3].yield_pct for weight, item in zip(similarity_weights, members)
        ) / weight_sum
        mean_z_score = sum(
            weight * item[3].z_score for weight, item in zip(similarity_weights, members)
        ) / weight_sum
    ...
    for rank, item in enumerate(ranked[:top_k], start=1):
        ...
        recommendations.append(LabelConditionRecommendation(
            rank=rank,
            recipe_id=recipe_id,
            score=round(score, 6),
            ...
            support=len(members),
            source_reaction_types=tuple(sorted({member[3].reaction_type for member in members})),
...
def recommend_conditions_from_labels(
    ...
    *,
    ...
    if top_k < 1:
        return LabelRecommendationResult(reaction_smiles, False, error="TOP_K_MUST_BE_POSITIVE")
    ...
    rows = tuple(
        row for row in load_label_index(records_path)
