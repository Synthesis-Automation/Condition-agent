...
SAVED_BATCH_WORKFLOW_SCHEMA_VERSION = "1.0"
...
def combine_saved_recommendation_batches(
    ...
    *,
    ...
    notify("review_started", "Writing the combined chemistry review CSV…")
    review_path = destination / "reaction_review.csv"
    try:
        export_concise_reaction_review_csv(
            records_path,
            review_path,
            progress_callback=lambda progress: notify(
                "review_rows", progress.message, progress.row_count
            ),
            cancel_check=cancel_check,
        )
    except ConciseReviewConversionCancelled as exc:
        raise RecommendationArtifactBuildCancelled(
            "Combine cancelled during review export"
        ) from exc
