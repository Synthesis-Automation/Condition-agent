...
def build_reaction_molecular_features(analysis: Any) -> ReactionMolecularFeatures:
    ...
    for number, edit in enumerate(edits):
        for atom in (edit.atom_1, edit.atom_2):
            if atom is not None and atom.side == "reactant":
                active.setdefault((atom.component_index, atom.atom_index), set()).add(number)
    partners = []
    for component in analysis.reactants:
        ...
        for environment in molecule.reactive_site_environments:
            ...
            locus = site.atom_indices if site.topology == "bond" else (environment.center_atom_index,)
            observed_atoms = tuple(sorted(index for index in locus if (component.component_index, index) in active))
            if not observed_atoms:
                continue
            edit_indices = tuple(sorted({
                number for atom in observed_atoms
                for number in active[(component.component_index, atom)]
            }))
            partners.append(ObservedSiteFeatures(
...
def validate_molecular_features(
    features: Mapping[str, Any], signature: Mapping[str, Any] | None = None,
) -> None:
    ...
    if features.get("schema_version") != MOLECULAR_FEATURE_SCHEMA_VERSION:
        raise ValueError("Incompatible molecular feature schema; regenerate converted records")
    if features.get("definition_versions") != dict(descriptor_definition_versions()):
        raise ValueError("Incompatible molecular feature definitions; regenerate converted records")
    ...
    for partner in features.get("partners") or ():
        ...
        if profile.get("schema_version") != "1.1" or dict(
            ...
            raise ValueError("Incompatible molecular profile provenance; regenerate converted records")
        if features.get("status") != "observed_edits" or not partner.get("edit_indices"):
