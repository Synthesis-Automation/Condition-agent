...
def recommend_from_reaction(
    ...
    *,
    ...
    by default. Complex substrate featurization is avoided in favor of reaction-level
    fingerprinting for better generality and maintainability.
    ...
    Args:
        ...
        use_fusion: Enable multi-source evidence fusion (precedents + analytics + rules + ML)
    ...
    # FUSION MODE: Use multi-source evidence fusion
    if use_fusion:
        try:
            from ..ml.fusion_recommender import recommend_with_fusion
            
            fusion_results = recommend_with_fusion(
            ...
            # Convert fusion format to core.py format for compatibility
            return _convert_fusion_to_core_format(
            ...
            # Fusion recommender not available, fall back to k-NN
            pass
        except Exception as e:
            # Fusion failed, fall back to k-NN
            import warnings
            warnings.warn(f"Fusion recommendation failed: {e}. Falling back to k-NN.")
    
    # STANDARD MODE: Use k-NN voting (default)
    ...
    # If user explicitly disables DRFP (use_drfp=False), they can provide features
    # or use legacy_features module
    features: Dict[str, Any] = {}
    
    # NOTE: Complex featurization is intentionally removed from core.
    # If needed, users can:
    # 1. Set use_drfp=False and provide features manually
    # 2. Use legacy_features module for family-specific analysis
    ...
    # 5) Vote for catalytic core (Laplace smoothing for better confidence)
