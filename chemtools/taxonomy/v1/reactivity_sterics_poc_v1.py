...
try:
    from .ar_context_sterics_v1 import analyze_smiles_ortho_counts, resolve_data_path
    from .reactivity_sterics_computed_v1 import compute_reactivity_sterics_poc_v1
except Exception:
    from ar_context_sterics_v1 import analyze_smiles_ortho_counts, resolve_data_path
    from reactivity_sterics_computed_v1 import compute_reactivity_sterics_poc_v1
...
def main():
    ...
    ap.add_argument(
        ...
        choices=["direct", "spec"],
        default="spec",
        help="Computation mode: 'direct' (module function) or 'spec' (computed-feature JSON).",
    )
    args = ap.parse_args()

    compiled = resolve_data_path(Path(args.compiled))
    groups = resolve_data_path(Path(args.groups))
    computed = resolve_data_path(Path(args.computed))

    if args.mode == "direct":
        results = [analyze_smiles_ortho_counts(s, compiled, groups) for s in args.smiles]
    else:
        results = [
            compute_reactivity_sterics_poc_v1(
                s,
                compiled_features_path=compiled,
                groups_path=groups,
                computed_spec_path=computed,
            )
