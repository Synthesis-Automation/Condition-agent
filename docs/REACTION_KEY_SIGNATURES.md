...
- **Reaction_Key** format: `Reacted -> Formed || Spectators`
- **Reactant signature (core)**: tokens from `reacted`
- **Reactant signature (ext)**: tokens from `reacted + spectators`
...
```
Reaction_Key: Ar-Br|R-NH2 -> Ar-NR2 || Pyridine
...
1) **Exact Reaction_Key** match  
2) **Core signature** match (reacted motifs only)  
3) **Reaction type** fallback (if neither key nor signature is available)
