...
def _read_dataset_aliases() -> Tuple[Dict[str, str], Dict[str, str]]:
    """Derive ligand alias maps from C-N coupling datasets, if present.
    
    Scans reaction datasets (C_N_Coupling_Cu.jsonl, etc.) to build
    CAS→canonical_name and normalized_name→canonical_name mappings for ligands.
...
def _pick_ligand_canonical(name: str, uid: str) -> Optional[str]:
    ...
    # Try reagent lookup for ligands
